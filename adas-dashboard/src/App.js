...
function LiveMode() {
      ...
      try {
        ...
        if (data.label === 'Distracted') {
          ...
          setConsecutiveDistracted(prev => {
            ...
            if (newCount === 3) playBeep();  // alert after 3 consecutive
            if (newCount === 5) playAlert(); // full alert after 5 consecutive
            return newCount;
          });
        } else {
  ...
  const bad = result?.label === 'Distracted';
  const rs  = result ? riskStyle(result.risk_level) : null;
  const attentivePct = frameCount > 0 ? Math.round(((frameCount - distractedCount) / frameCount) * 100) : 0;
  const distractedPct = frameCount > 0 ? Math.round((distractedCount / frameCount) * 100) : 0;
        ...
        <div className="slide-in">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem', marginBottom: '1.25rem' }}>
            {[
              ...
              { label: 'Attentive',  value: `${attentivePct}%`,  color: '#38a169', bg: '#f0fff4', border: '#9ae6b4' },
              { label: 'Distracted', value: `${distractedPct}%`, color: bad ? '#c53030' : '#555', bg: bad ? '#fff5f5' : '#fafafa', border: bad ? '#feb2b2' : '#e8e8e8' },
              { label: 'Risk Level', value: result.risk_level, color: rs.color, bg: rs.bg, border: rs.border },
            ].map(({ label, value, color, bg, border }) => (
              <div key={label} style={{ background: bg, border: `1px solid ${border}`, borderRadius: '10px', padding: '1.1rem 1.25rem' }}>
                <div style={{ fontSize: '0.75rem', color: '#888', marginBottom: '6px', fontWeight: 500 }}>{label}</div>
                <div style={{ fontSize: label === 'Risk Level' || label === 'Frames' ? '1.2rem' : '2rem', fontWeight: 700, color, lineHeight: 1 }}>{value}</div>
              </div>
            ))}
          </div>
