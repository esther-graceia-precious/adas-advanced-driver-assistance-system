...
import LiveModeDemo from './LiveModeDemo';
...
function MultiStreamTags({ ms }) {
  ...
  return (
    <div style={{ marginTop: '0.75rem', display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
      {faceFound ? (
        <>
          <span style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: '6px', padding: '3px 10px', fontSize: '0.72rem', color: '#1d4ed8' }}>👤 {ms.head}</span>
          <span style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '6px', padding: '3px 10px', fontSize: '0.72rem', color: '#166534' }}>👁️ {ms.eye}</span>
          <span style={{ background: '#fdf4ff', border: '1px solid #e9d5ff', borderRadius: '6px', padding: '3px 10px', fontSize: '0.72rem', color: '#7e22ce' }}>👄 {ms.mouth}</span>
        </>
      ) : (
        <span style={{ background: '#fff9f5', border: '1px solid #ff6600', borderRadius: '6px', padding: '3px 10px', fontSize: '0.72rem', color: '#ff6600' }}>👤 Face not detected in this frame</span>
      )}
      {ms.reasons && ms.reasons.length > 0 && ms.reasons.map((r, i) => (
        <span key={i} style={{ background: '#fff5f5', border: '1px solid #feb2b2', borderRadius: '6px', padding: '3px 10px', fontSize: '0.72rem', color: '#c53030', fontWeight: 600 }}>⚠️ {r}</span>
      ))}
    </div>
  );
}
